<template>
  ...
  > 
    <v-system-bar style="max-width:900px"  window :color="barColor" max-width="900">
      <span class="text-truncate">Navegador </span>
      <v-spacer></v-spacer>
      <v-icon>remove</v-icon>
      <v-icon>check_box_outline_blank</v-icon>
      <v-icon>close</v-icon>
    </v-system-bar>
    <div :class="{'font-cutive-mono':true,'zoom-in':true}" style="background-color:white; height:200px; font-size:18px;">
        <span style="color:white">hhhhhh</span>
      <span style="color:black; border: 1px solid black; border-radius:10px;">usuarioLinux@usuario-pc-3543:~{{this.prompt}}</span> <span :class="this.customClass" ></span>
    </div>
...
export default {
  ...
  data(){
    return{
      barColor: this.Color ? this.Color : '#1E90FF',
      typed:true,
